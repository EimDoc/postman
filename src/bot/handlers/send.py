...
from src.bot.keyboards.all_keyboards import accepted_kb, rejected_kb, choose_rephrase_way_kb
...
# @send_router.callback_query(F.data == "gpt")
...
async def enter_header(query: types.CallbackQuery, state: FSMContext):
    ...
    await query.message.reply("Введите заголовок новости:")
    await query.answer()
...
async def enter_text(message: types.Message, state: FSMContext):
    ...
    await message.reply("Введите текст новости:")
